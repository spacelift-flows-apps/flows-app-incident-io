...
const privateIncidentIncidentCreatedV2Subscription: AppBlock = {
  name: "Private Incident - Incident Created V2",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "private_incident.incident_created_v2") {
      console.log("Received private_incident.incident_created_v2 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "privateIncidentIncidentCreatedV2Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a private_incident.incident_created_v2 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default privateIncidentIncidentCreatedV2Subscription;
