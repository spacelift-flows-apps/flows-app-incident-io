...
const privateIncidentFollowUpCreatedV1Subscription: AppBlock = {
  name: "Private Incident - Follow Up Created V1",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "private_incident.follow_up_created_v1") {
      console.log("Received private_incident.follow_up_created_v1 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "privateIncidentFollowUpCreatedV1Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a private_incident.follow_up_created_v1 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default privateIncidentFollowUpCreatedV1Subscription;
