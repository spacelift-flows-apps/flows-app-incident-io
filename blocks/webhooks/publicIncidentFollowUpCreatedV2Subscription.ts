...
const publicIncidentFollowUpCreatedV2Subscription: AppBlock = {
  name: "Public Incident - Follow Up Created V2",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "public_incident.follow_up_created_v2") {
      console.log("Received public_incident.follow_up_created_v2 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "publicIncidentFollowUpCreatedV2Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a public_incident.follow_up_created_v2 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default publicIncidentFollowUpCreatedV2Subscription;
