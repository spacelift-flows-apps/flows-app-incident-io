...
const publicIncidentFollowUpCreatedV1Subscription: AppBlock = {
  name: "Public Incident - Follow Up Created V1",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "public_incident.follow_up_created_v1") {
      console.log("Received public_incident.follow_up_created_v1 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "publicIncidentFollowUpCreatedV1Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a public_incident.follow_up_created_v1 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default publicIncidentFollowUpCreatedV1Subscription;
