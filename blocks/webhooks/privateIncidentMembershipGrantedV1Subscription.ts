...
const privateIncidentMembershipGrantedV1Subscription: AppBlock = {
  name: "Private Incident - Membership Granted V1",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "private_incident.membership_granted_v1") {
      console.log("Received private_incident.membership_granted_v1 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "privateIncidentMembershipGrantedV1Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a private_incident.membership_granted_v1 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default privateIncidentMembershipGrantedV1Subscription;
