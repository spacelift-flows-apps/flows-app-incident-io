...
const publicIncidentIncidentUpdatedV2Subscription: AppBlock = {
  name: "Public Incident - Incident Updated V2",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "public_incident.incident_updated_v2") {
      console.log("Received public_incident.incident_updated_v2 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "publicIncidentIncidentUpdatedV2Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a public_incident.incident_updated_v2 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default publicIncidentIncidentUpdatedV2Subscription;
