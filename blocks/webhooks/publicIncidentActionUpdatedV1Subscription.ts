...
const publicIncidentActionUpdatedV1Subscription: AppBlock = {
  name: "Public Incident - Action Updated V1",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "public_incident.action_updated_v1") {
      console.log("Received public_incident.action_updated_v1 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "publicIncidentActionUpdatedV1Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a public_incident.action_updated_v1 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default publicIncidentActionUpdatedV1Subscription;
