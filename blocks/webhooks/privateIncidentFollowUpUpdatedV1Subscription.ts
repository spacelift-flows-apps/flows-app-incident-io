...
const privateIncidentFollowUpUpdatedV1Subscription: AppBlock = {
  name: "Private Incident - Follow Up Updated V1",
  ...
  async onInternalMessage(input) {
    ...
    if (event && event.event_type === "private_incident.follow_up_updated_v1") {
      console.log("Received private_incident.follow_up_updated_v1 webhook event");
      await events.emit(event);
    } else {
      console.warn(
        "privateIncidentFollowUpUpdatedV1Subscription received unexpected event type:",
        event?.event_type
      );
    }
  },
  ...
  outputs: {
    default: {
      name: "Event",
      description: "Emitted when a private_incident.follow_up_updated_v1 webhook is received",
      default: true,
      type: {
      "type": "object",
      "properties": {
...
export default privateIncidentFollowUpUpdatedV1Subscription;
